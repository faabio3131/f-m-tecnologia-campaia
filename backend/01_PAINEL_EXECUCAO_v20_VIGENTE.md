...
**Atualizado em:** 19/09/2026 (PONTO ZERO WEB E SYSTEM DESIGN COMPLETO — produzidos em
`docs/web/00_INDICE_PONTO_ZERO_WEB.md` a `07_CERTIFICACAO_PONTO_ZERO_WEB.md`, mais 4 ADRs propostas
(`docs/14_ADR_0016...md` a `docs/17_ADR_0019...md`). Nenhuma ADR aprovada, nenhuma implementação de frontend
iniciada. Estado: `SYSTEM DESIGN WEB PRODUZIDO — PENDENTE DE REVISÃO E APROVAÇÃO HUMANA`. Mobile permanece em
quarentena arquitetural, sem evolução)
...
> **VERSÃO VIGENTE.** Substitui `01_PAINEL_EXECUCAO_v19_VIGENTE.md` (e as anteriores v1-v18). Versões antigas
...
| Campo | Valor |
|---|---|
| **Fase atual** | **Ponto Zero Web e System Design completo APROVADOS** (19/09) — as 4 ADRs (`docs/14_ADR_0016...md` a `docs/17_ADR_0019...md`) foram aprovadas pelo Diretor Fábio Aluizio da Silva, e o Product Charter/NFR foram reconciliados literalmente com a Lei Web First. **Nenhuma implementação de frontend iniciada** — WP-01 permanece não iniciado por instrução explícita da mesma autorização. App mobile permanece em **QUARENTENA ARQUITETURAL**, intocado. Próximo passo: validação completa + CI verde no novo HEAD, transição da PR de Draft para Ready, e merge controlado; após o merge, esta execução PARA — nenhum Work Package começa nesta missão |
| **Última tarefa concluída** | Aprovação arquitetural (ADR-0016–0019: PROPOSTA → APROVADA) e reconciliação literal do Product Charter/NFR com a Lei Web First, por autorização humana atual e explícita (19/09/2026). Ver "Registro de blocos executados" desta versão |
| **Status** | Backend/domínio **inalterado nesta execução** (nenhuma alteração em `backend/campaia_core/`, `backend/api/`, `backend/db/`, `contracts/`); **System Design Web APROVADO**; **app mobile em QUARENTENA ARQUITETURAL**, sem alteração nesta sessão; **bloco fiscal FISC V2-16.5** permanece importado e formalmente bloqueado, sem alteração nesta sessão |
| **Última evidência** | `docs/web/07_CERTIFICACAO_PONTO_ZERO_WEB.md` — CI real do GitHub confirmado três vezes: run #11 no HEAD `0160182`, run #12 no HEAD `4ae62a5`, e **run #13 (`completed`/`success`) no HEAD `30903fa`**, todas `completed`/`success`. Quatro ADRs (0016–0019) agora **APROVADA**. `30903fa` não é declarado aqui como HEAD final — esta execução adiciona commits documentais sobre ele; ver histórico do Git e o relatório final da missão para o HEAD atual |
| **Bloqueios** | Nenhum bloqueio técnico novo. Pendências reais remanescentes (não bloqueiam as ADRs, que já estão aprovadas): provedor específico de identidade (WP-02), serviço de container específico (Work Package de CI/CD), orçamento mensal exato e confirmação jurídica de residência de dados (D-08), destino final do `mobile/`. Bloco fiscal e mobile mantêm seus bloqueios já registrados nas versões anteriores deste painel |
| **Próxima ação** | Validação completa + CI verde no HEAD desta execução; transição da PR de Draft para Ready; merge controlado vinculado ao HEAD certificado. Após o merge: PARAR — nenhum Work Package (WP-01 incluído) inicia nesta missão; próximo passo depende de um Prompt Mestre de implementação Web dedicado |
| **Gate para avançar** | Gate 1 (Arquitetura) — **aprovado** (19/09/2026); Gate 2 (Scaffold Web/WP-01) aguarda um Prompt Mestre de implementação futuro, não iniciado nesta execução |
...
| Ponto Zero Web (8 documentos) | `docs/web/00_INDICE_PONTO_ZERO_WEB.md` a `07_CERTIFICACAO_PONTO_ZERO_WEB.md` | **PRODUZIDO, ARQUITETURA APROVADA, AGUARDANDO CI E MERGE CONTROLADO** (19/09) |
...
| P-25 | System Design Web produzido mas **nenhuma ADR aprovada** (0016–0019); nenhum Work Package pode começar | Bloqueia o início de qualquer implementação de frontend até revisão e aprovação humana das ADRs |
...
| P-27 | Autenticação real ainda não existe (`api/deps.py` usa token fixo de teste) | Maior superfície de risco do roadmap; WP-02 é o Work Package que resolve, condicionado à aprovação de ADR-0018 |
