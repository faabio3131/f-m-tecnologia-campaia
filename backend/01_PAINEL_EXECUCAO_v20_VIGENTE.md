...
| **Última evidência** | `docs/web/07_CERTIFICACAO_PONTO_ZERO_WEB.md` — validação local de runtime no SHA `a4a36ec`; CI real do GitHub (run #11, `completed`/`success`) confirmado no HEAD `0160182` (HEAD da PR #4 no momento de sua criação, **não** o HEAD final após a correção de referências de ADR/D-08/token — ver relatório da correção para o HEAD atual) |
