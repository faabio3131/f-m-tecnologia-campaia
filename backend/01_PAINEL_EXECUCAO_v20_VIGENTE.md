...
| Campo | Valor |
|---|---|
| **Fase atual** | **Ponto Zero Web e System Design completo PRODUZIDOS** (19/09) em `docs/web/` (8 documentos) + 4 ADRs propostas (`docs/14_ADR_0016...md` a `docs/17_ADR_0019...md`). Nenhuma ADR aprovada. Nenhuma implementação de frontend iniciada. App mobile permanece em **QUARENTENA ARQUITETURAL**, intocado. Próximo passo: revisão e aprovação humana das 4 ADRs propostas, começando por ADR-0017 (framework) para destravar o Gate 2 (Scaffold Web) |
| **Última tarefa concluída** | System Design Web: CURRENT reconstruído por leitura direta de código (não por citação de painel), matriz CURRENT×TARGET, bounded contexts, diagramas de arquitetura (Mermaid), fluxos críticos, modelo de dados TARGET, matriz de ameaças (STRIDE), roadmap reconciliado com 5 Work Packages prontos, 4 ADRs propostas, certificação com matriz de validação real. Ver "Registro de blocos executados" desta versão |
| **Status** | Backend/domínio **inalterado e revalidado nesta sessão** — 267 testes de domínio + 81 de API + AsyncAPI 24/24 + DDL/RLS contra Postgres real, todos reexecutados de fato no HEAD desta missão; **System Design Web PRODUZIDO, NÃO APROVADO**; **app mobile em QUARENTENA ARQUITETURAL**, sem alteração nesta sessão; **bloco fiscal FISC V2-16.5** permanece importado e formalmente bloqueado, sem alteração nesta sessão |
| **Última evidência** | `docs/web/07_CERTIFICACAO_PONTO_ZERO_WEB.md` — validação local de runtime no SHA `a4a36ec`; CI real do GitHub confirmado duas vezes: run #11 (`completed`/`success`) no HEAD `0160182` (HEAD da PR #4 no momento de sua criação), e **run #12 (`completed`/`success`) no HEAD `4ae62a5`**, após a correção de referências de ADR/D-08/token. Quatro ADRs (0016–0019) continuam **PROPOSTA**; nenhuma implementação Web foi iniciada. `4ae62a5` não é declarado aqui como HEAD final da PR — commits documentais adicionais podem ter sido feitos depois; ver histórico do Git para o HEAD atual |
| **Bloqueios** | Nenhum bloqueio técnico novo. 4 ADRs aguardam decisão humana explícita antes de qualquer Work Package poder começar (WP-01 depende de ADR-0017; WP-02 depende de ADR-0018). Bloco fiscal e mobile mantêm seus bloqueios já registrados nas versões anteriores deste painel |
| **Próxima ação** | Revisão humana da PR `docs(web): Ponto Zero e System Design completo do CampaIA` e decisão sobre as 4 ADRs propostas. Nenhuma implementação deve começar antes dessa aprovação |
| **Gate para avançar** | Gate 1 (Arquitetura) — System Design produzido, aguardando revisão; Gate 2 (Scaffold Web) depende da aprovação de ADR-0017 |
...
| ADR-0016 precedência Web First vs. Charter | `docs/14_ADR_0016_LEI_WEB_FIRST_VS_NATUREZA_MOBILE_CHARTER.md` | PROPOSTA (19/09) |
| ADR-0017 framework de frontend Web | `docs/15_ADR_0017_FRAMEWORK_FRONTEND_WEB.md` | PROPOSTA (19/09) |
| ADR-0018 autenticação e sessão Web | `docs/16_ADR_0018_AUTENTICACAO_SESSAO_WEB.md` | PROPOSTA (19/09) |
| ADR-0019 implantação TARGET | `docs/17_ADR_0019_IMPLANTACAO_TARGET.md` | PROPOSTA (19/09) |
| Ponto Zero Web (8 documentos) | `docs/web/00_INDICE_PONTO_ZERO_WEB.md` a `07_CERTIFICACAO_PONTO_ZERO_WEB.md` | **PRODUZIDO, PENDENTE DE REVISÃO E APROVAÇÃO HUMANA** (19/09) |
