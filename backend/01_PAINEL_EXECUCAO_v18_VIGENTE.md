...
**Atualizado em:** 05/09/2026 (B8 — Especificação das telas do app — **CONSTRUÍDO**: `docs/product/13_ESPECIFICACAO_TELAS_APP.md`, incluindo pesquisa de mercado de concorrentes, o fluxo completo de nova campanha e a especificação do Modo Manual/Automático de autonomia — ver "Decisões registradas" e o registro de blocos executados desta versão para a decisão de escopo de 05/09 que trouxe o Nível 2 de autonomia para dentro do MVP)
...
> **VERSÃO VIGENTE.** Substitui `01_PAINEL_EXECUCAO_v17_VIGENTE.md` (e as anteriores v1-v16) no Drive. Se as
...
| Campo | Valor |
|---|---|
| **Fase atual** | Fase 3 em execução, avançando para a Fase 8 (app mobile); camada BFF/API conforme ao contrato literal, com persistência real opcional (SQLite); DDL do Postgres construído e verificado contra um servidor real (B7); catálogo de eventos formalizado como contrato AsyncAPI 3.0; catálogo de erros em linguagem de usuário publicado e com cobertura 100% do Connector Hub; motor de otimização e pacing (B5) construído como camada de sugestão apenas; repositório Git + CI (C1) construído, verificado e entregue ao PC do Diretor; especificação de telas do app (B8) concluída; tudo genuinamente salvo no Drive e commitado no repositório Git |
| **Última tarefa concluída** | B8 — Especificação das telas do app: `docs/product/13_ESPECIFICACAO_TELAS_APP.md` construído com pesquisa real de mercado de concorrentes (Madgicx, Revealbot, Smartly.io, AdCreative.ai, Meta Advantage+), cobrindo F1–F10 (onboarding, brand kit, dashboard, fluxo completo de nova campanha, detalhe de campanha, otimizações, configurações, auditoria); durante a revisão, o Diretor decidiu trazer o Nível 2 de autonomia (Modo Automático, limite definido pelo cliente + senha de administrador) para dentro do MVP, decisão registrada em `docs/product/DECISOES_DIRETOR.md` item 5 e propagada de forma consistente para `CAMPAIA_PRODUCT_CHARTER.md` e `OUT_OF_SCOPE.md`; commitado no repositório (`docs: especificações de telas B8 e decisões do diretor`) |
| **Status** | Núcleo de domínio determinístico **VALIDADO** (263/263, inalterado); camada BFF/API **VALIDADA** com persistência real opcional, P-19 corrigida e achado 18 (`BudgetPatchRequest`, alias `daily_cap`/`new_daily_cap`) corrigido (80/80, inalterada); DDL do Postgres (B7) **CONSTRUÍDO E VERIFICADO CONTRA SERVIDOR REAL**; catálogo de eventos **FORMALIZADO** em AsyncAPI 3.0 (24 eventos); catálogo de erros de usuário **PUBLICADO E COMPLETO**; motor de otimização e pacing (B5) **CONSTRUÍDO**; repositório Git + CI (C1) **100% OPERACIONAL** — publicado em `https://github.com/faabio3131/CampaIA` (privado), CI rodando e verde a cada push; **B8 (especificação das telas do app) CONCLUÍDO**; as 5 decisões do Diretor (D-03/D-05/D-06/D-08/D-09) estão **RESOLVIDAS** desde 27/08/2026 (ver "Decisões registradas") |
| **Última evidência** | `docs/product/13_ESPECIFICACAO_TELAS_APP.md` (especificação completa do B8) e `docs/product/DECISOES_DIRETOR.md` item 5 (decisão de escopo do Modo Automático, com citações literais do Diretor) |
| **Bloqueios** | Nenhum. Nenhuma decisão do Diretor pendente. Residual não bloqueante: P-11 (preço exato do Memorystore/Redis em São Paulo) e confirmação formal da emissão do CNPJ da F&M (em andamento com a contabilidade do Diretor) |
| **Próxima ação** | Recomendação do engenheiro sênior (eu): com B8 concluído, o próximo bloco destravado é a **implementação real das telas do app mobile** (Fase 8) a partir da especificação de `13_ESPECIFICACAO_TELAS_APP.md` — começando pelo caminho crítico (onboarding + fluxo de nova campanha), já que sem eles nenhuma campanha pode ser criada. Ver seção "Blocos ainda construíveis sem PC" |
...
| — | Achado 18 — `BudgetPatchRequest` (`api/models.py`) não aceitava o nome de campo `daily_cap` exigido pelo contrato (`bff-openapi.yaml`), só `new_daily_cap` | **CORRIGIDO** (05/09) — alias `daily_cap`/`new_daily_cap` com `populate_by_name=True`; ambos os nomes validam, `extra="forbid"` preservado; 80/80 testes de API confirmados sem regressão |

**Recomendação de engenharia:** com B8 concluído, a especificação de telas está pronta para orientar a
**implementação real do app mobile** (Fase 8) — recomendo o caminho crítico (onboarding + fluxo de nova
campanha) como próximo bloco de construção, por ser o pré-requisito para qualquer campanha poder ser criada.
