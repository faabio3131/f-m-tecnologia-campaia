...
**Atualizado em:** 05/09/2026 (C1 agora **100% operacional**: repositório publicado em `https://github.com/faabio3131/CampaIA` — privado —, com o GitHub Actions rodando de fato a cada push. Um erro real de CI foi encontrado na primeira execução real no GitHub — `ModuleNotFoundError: No module named 'httpx'`, dependência de `starlette.testclient` ausente de `backend/requirements.txt`, mascarada localmente porque a sandbox já tinha `httpx` instalado por outro motivo — diagnosticado a partir do log real do Actions, corrigido, e a execução seguinte confirmada com sucesso via `gh run view --json status,conclusion`)
...
> **VERSÃO VIGENTE.** Substitui `01_PAINEL_EXECUCAO_v16_VIGENTE.md` (e as anteriores v1-v15) no Drive. Se as
...
| **Última tarefa concluída** | C1 — CI corrigido e verde de fato no GitHub: diagnosticado (via `gh run view --log-failed`) e corrigido o `ModuleNotFoundError: No module named 'httpx'` que fazia a suíte de API falhar no ambiente limpo do Actions; `httpx==0.28.1` adicionado a `backend/requirements.txt` (versão confirmada real no PyPI antes de fixar); push direto pelo terminal do Diretor (`gh`/`git` já autenticados); execução seguinte confirmada com sucesso (`"conclusion": "success"`) |
| **Status** | Núcleo de domínio determinístico **VALIDADO** (263/263, inalterado); camada BFF/API **VALIDADA** com persistência real opcional e P-19 corrigida (80/80, inalterada); DDL do Postgres (B7) **CONSTRUÍDO E VERIFICADO CONTRA SERVIDOR REAL**; catálogo de eventos **FORMALIZADO** em AsyncAPI 3.0 (24 eventos); catálogo de erros de usuário **PUBLICADO E COMPLETO**; motor de otimização e pacing (B5) **CONSTRUÍDO**; repositório Git + CI (C1) **100% OPERACIONAL** — publicado em `https://github.com/faabio3131/CampaIA` (privado), CI rodando e verde a cada push; as 5 decisões do Diretor (D-03/D-05/D-06/D-08/D-09) estão **RESOLVIDAS** desde 27/08/2026 (ver "Decisões registradas") |
| **Última evidência** | `docs/evidence/EVIDENCIA_C1_CI_HTTPX_FIX_20260905.md` (diagnóstico do log real do Actions, correção do `requirements.txt`, confirmação de sucesso da execução seguinte via `gh run view --json status,conclusion`) |
...
| **Próxima ação** | Recomendação do engenheiro sênior (eu): com D-03 resolvida (segmento = qualquer pequeno negócio local), **B8** (especificação das telas do app) está **desbloqueado** — recomendo como próximo bloco de construção. Ver seção "Blocos ainda construíveis sem PC" |
...
**Recomendação de engenharia:** com D-03 resolvida e o C1 100% operacional, **B8** (especificação das telas do
app) está desbloqueada e é a recomendação para o próximo bloco de construção.
