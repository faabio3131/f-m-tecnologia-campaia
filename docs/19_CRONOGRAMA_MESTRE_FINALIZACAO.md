...
| 1.3 | Autenticação real da API (hoje é token fixo de dev) — substituir por sessão real, alinhado ao provedor OAuth/OIDC já aprovado na ADR-0018 para o Web | **CONCLUÍDO (24/09/2026, WP-02)** | Provedor: Google Identity Platform (ADR-0022). ID token verificado via `firebase-admin`; sessão server-side com cookie `HttpOnly`/`Secure`/`SameSite=Lax`, CSRF double-submit (incl. mitigação de login-CSRF via nonce de pre-login), TTL 24h sem refresh silencioso, anti-fixation, fail-closed (fixture de dev impossível fora de `test`/`dev-local` por construção). 496 testes verdes (366 domínio + 130 API), `fm-security-review` executado, zero achado bloqueante. Ver `docs/evidence/EVIDENCIA_AUTENTICACAO_REAL_20260924.md` |
...
| 2.2 | Autenticação de sessão Web (cookies `HttpOnly`/`Secure`/`SameSite`, CSRF) — ADR-0018 | TARGET — backend **CONCLUÍDO (1.3/WP-02)**, falta o frontend consumir | O backend já expõe `GET /auth/login-nonce`, `POST/DELETE /auth/session`, `GET /me` (com `csrf_token`); falta o scaffold do frontend (2.1/WP-01) para chamar esses endpoints de verdade |
