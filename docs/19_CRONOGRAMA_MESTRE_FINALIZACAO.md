...
| 1.3 | Autenticação real da API (hoje é token fixo de dev) — substituir por sessão real, alinhado ao provedor OAuth/OIDC já aprovado na ADR-0018 para o Web | TARGET — provedor **DECIDIDO (24/09/2026)**, integração ainda não construída | Provedor: Google Identity Platform, plano gratuito (ver ADR-0022). Construção da integração real (WP-02) ainda não iniciada — depende também de 1.6 (projeto Google Cloud provisionado) |
