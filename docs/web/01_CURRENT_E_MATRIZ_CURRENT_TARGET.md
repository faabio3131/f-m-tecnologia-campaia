...
| 8 | AI Model Gateway (proveniência com `Attempt`/`Provenance` em toda chamada, sucesso ou falha; `CostLedger` debita sempre que a chamada chegou ao provedor, mesmo com resposta imprestável; `OutputSchema.validate` rejeita saída fora do contrato, nunca remenda; seleção de provedor por `Prefer` COST/LATENCY/QUALITY; circuit breaker abre após `CIRCUIT_THRESHOLD=3` falhas consecutivas por provedor; `ModerationBlocked` não abre circuito e não tenta outro provedor — retorna imediatamente; trava de credencial `assert_no_credentials` antes de qualquer chamada externa) | `campaia_core/ai_gateway.py` | **LIDO INTEGRALMENTE nesta correção** (436 linhas), IMPLEMENTADO, TESTADO (`test_ai_gateway.py`, 42 testes citados no painel, não reexecutados nominalmente nesta correção — cobertos pela suíte completa de 267 testes de domínio validada na Etapa 9) | leitura completa; teste arquitetural citado no docstring que falha se `connectors`/`saga` forem importados aqui — presença do teste confirmada por nome de arquivo (`tests/`), execução do teste específico não isolada nesta correção |
| 9 | Sanitização de PII (pseudonimização, não exclusão; mapa de reidratação nunca sai do backend, nunca vai a log); agentes (funções puras, recusam contexto obrigatório ausente, nunca supõem); simulador de IA (comportamentos programáveis: OK/TIMEOUT/ERROR/MODERATION/BAD_SCHEMA/EXPENSIVE) | `sanitizer.py`, `agents.py`, `ai_simulator.py` | **LIDO INTEGRALMENTE nesta correção**, IMPLEMENTADO, TESTADO | leitura completa dos 3 arquivos |
| 10 | Outbox (em memória, entrega pelo menos uma vez, máximo 5 tentativas, `DEAD_LETTER` ao esgotar, ordenação garantida só por agregado, não globalmente); Inbox (dedupe por `tenant_id`+`event_id`); Webhooks (HMAC-SHA256 sobre timestamp+corpo cru, comparação em tempo constante via `hmac.compare_digest`, janela padrão de 5 minutos, dedupe por `provider`+`external_event_id`, `secret_resolver` por provider — hoje sem persistência confirmada, `_seen` é um `set` em memória); Reconciliação (nunca amplia efeito — só `PAUSE_EXTERNAL`/`UPDATE_INTERNAL_STATE`/`ADOPT_EXTERNAL` são auto-aplicáveis; estado externo é autoridade para estado da plataforma; intenção e orçamento autorizado permanecem internos; recurso órfão = severidade `FINANCIAL` + `requires_human=True`; divergência de orçamento = `ESCALATE_HUMAN` + `requires_human=True`, nunca ajustado automaticamente) | `outbox.py`, `webhooks.py`, `reconciliation.py` | **LIDO INTEGRALMENTE nesta correção**, IMPLEMENTADO, TESTADO | leitura completa dos 3 arquivos. `PENDÊNCIA` real (não resolvida por esta leitura): nenhuma persistência de outbox/webhooks foi encontrada além de memória de processo — avaliação de escopo de persistência e de tenant/account para webhooks fica para Work Package futuro, não promovida a prontidão de produção |
